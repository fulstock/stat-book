...
Авторский коллектив:
\begin{compactlist}
    \item Главный редактор (теория вероятностей)~---~Рожков~И.\,О.
...
\begin{enumerate}
    ...
    $ \Rightarrow \myprob{\bigcup\limits_{i=1}^\infty A_i} = \sum\limits_{i=1}^\infty \myprob{A_i}$ ({\it счётная аддитивность~--- вероятность счётного объединения несовместных событий равна сумме их вероятностей})
\end{enumerate}
\end{defn}
...
\begin{defn}
    Пусть $\Omega = \{w_1, w_2,\ldots, w_n\}$~--- конечное непустое множество, $\mathcal{F}$~--- множество всех подмножеств $\Omega$. Положим $\myprob{\{w_i\}} = p_i$. Вероятностное пространство, определённое таким образом,~--- {\it дискретное вероятностное пространство}. Тогда для любого события $A = \{w_1, \ldots, w_k\}$ его вероятность $\myprob{A} = \sum\limits_{i=1}^k p_i$.
\end{defn}
...
\begin{defn}
    Пусть есть вероятностное пространство $(\Omega, \mathcal{F}, \mathbb{P})$. События $A_1, \ldots, A_n \in \mathcal{F}$ называются {\it независимыми в совокупности}, если $\forall k \in \overline{2, n}$ и $\forall i_{1}, \ldots, i_{k} \colon 1 \leqslant i_1 \leqslant i_2 \leqslant \ldots \leqslant i_k \leqslant n$ выполняется 
    \begin{equation*}
...
\begin{namedthm}[Критерий независимости]
    ...
    \begin{equation*}
        \mathbb{P}\left( \bigcap_{i=1}^{n} A_{i}^{\left( \delta_{j} \right)} \right)
...
\begin{defn}
    $(\Omega, \mathcal{A})$ и $(\Omega, \mathcal{F})$~--- {\it измеримые пространства}, элементы $\mathcal{A}$ и $\mathcal{F}$~---{\it измеримые множества}.
\end{defn}
...
\begin{rmrk}
    Борелевская функция измерима относительно борелевской ${\sigma \text{-алгебры}}$.
\end{rmrk}

\subsubsection{Случайные величины}
\begin{defn}
    Пусть даны $(\Omega, \mathcal{F})$~--- измеримое пространство и $(\mathbb{R}, \mathfrak{B})$, где $\mathfrak{B}$~--- борелевская ${\sigma \text{-алгебра}}$ множеств на числовой прямой $\mathbb{R}$. Тогда измеримая относительно $\mathcal{F}$ функция $\xi: \Omega \to \mathbb{R}$ называется {\it случайной величиной}.
\end{defn}
...
\begin{enumerate}
    ...
    \item
        ...
        \begin{multline*}
            ...
            = \mathbb{P}\left( x_0 - \frac{1}{n} \leqslant \xi \leqslant x_0 \right) \xrightarrow[n \to +\infty]{} 0
                \end{multline*}
...
\begin{enumerate}
    ...
    \item $f_\xi(x) \geqslant 0~ \forall x$ (неотрицательность);
...
\begin{rmrk}
    Слагаемое $-3$ добавлено, чтобы коэффициент эксцесса стандартного нормального распределения был равен нулю. Иногда его не учитывают и считают, что коэффициент эксцесса $\mathbf{N}(0,1)$ равен 3.
\end{rmrk}
...
\begin{defn}
    Пусть, для простоты, функция распределения $F$ непрерывна и строго монотонна. Тогда {\it квантилем} уровня (порядка) $\delta$, где $\delta \in (0; 1), $ называется решение $x_\delta$ уравнения $F(x_\delta) = \delta.$ 
\end{defn} 
...
\begin{proof}
    \begin{itemize}
        \item[$\text{(r)} \Rightarrow \text{p}$] Из обобщённого неравенства Чебышёва:
    \begin{equation*}
