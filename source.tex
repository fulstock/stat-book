...
\begin{defn}
    ...
    \begin{equation*}
        F_\xi(x) = \mathbb{P}(\xi Б x) = \int\limits_{-\infty}^x f(t)dt
    \end{equation*}
\end{defn}
\begin{rmrk}
...
\begin{proof}
    ...
    \item[$\text{(r)} \nLeftarrow \text{п.н.}$] См. предыдущий пример.
...
\begin{rmrk}
    Во многих источниках слабая сходимость и сходимость по распределению вводятся как один и тот же вид сходимости. Те немногие доказательства эквивалентности, найденные авторами, либо требуют знания теории меры, либо являются слишком кринжовыми, чтобы включать их в эту методичку.
\end{rmrk}
...
\begin{enumerate}
    ...
    \begin{enumerate}[label={\arabic*)}]
        ...
        * Пользуясь полученной оценкой на дисперсию, можно показать, что эмпирическая функция распределения сходится к истинной по распредедению:
        $$ \myprob{|F_n^*(y) - F(y)|}
        $$
    \end{enumerate}
    \item Применим ЦПТ:
...
\begin{defn}
{\it Несмещённая оценка} парамаетра $\theta$~--- статистика $T(\mathbf{X})$, т.ч. $\forall \theta \in \Theta: \mathbb{E}T(X) = \theta$.
\end{defn}
...
\begin{aligned}
    ...
    \Leftrightarrow \\
    \Leftrightarrow T(\mathbf{X}) = a T(\mathbf{X})+b~ \forall \theta \Rightarrow a = 1, b = 0
\end{aligned}
\end{equation*}
...
\begin{exmp}
    ...
    \begin{multline*}
        \mathbb{E} \varphi(X_{(n)})=\theta^{-n} n \int\limits_{0}^{\theta} \varphi(t) t^{n-1} d t=0 \Rightarrow \int\limits_{a}^{b} f(t) t^{n-1} d t=0~ \forall a,b \in \mathbb{R} \Rightarrow \\
        \Rightarrow \int\limits_{B} f(t) t^{n-1} d t=0 \quad \forall B \in \mathfrak{B}
    \end{multline*}
    Таким образом, $f(t) t^{n-1} \stackrel{\text{п.н.}}{=} 0$ при $t > 0$, и указанная статистика является полной.
\end{exmp}
...
\begin{equation*}
    \int T(x) L(x, \theta) \mu(d x)=\mathbb{E} T(\mathbf{X})=T(\mathbf{X}) \Rightarrow \int T(x) \cfrac{\partial L(x, \theta)}{\partial \theta} \mu(d x)=(\theta)^{*}
\end{equation*}
...
\begin{equation*}
    \mathbb{E}(T(\mathbf{X})-T(\mathbf{X})) U(X, \theta)=\theta^{*}
\end{equation*}
...
\begin{equation*}
    T(\mathbf{X})=\varphi(\theta) U(X, \theta)+\psi(\theta) \Rightarrow T(\mathbf{X})=\psi(\theta) \Rightarrow a_{n}(\theta)=\varphi(\theta)
\end{equation*}
...
\begin{equation*}
    \cfrac{\sqrt{n}(\theta^{*}-\theta)}{\sigma^{2}(\theta)} \Rightarrow \mathbf{N}(0, 1).
\end{equation*}
\end{defn}
