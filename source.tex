...
\begin{proof}
    Проверим, что прообраз любого борелевского множества при отображении $g(\xi) \colon \Omega \mapsto \mathbb{R}$ является событием. Возьмём произвольное $B \in \mathfrak{B}(\mathbb{R})$ и положим $B_1 = g^{-1}(B)$. Множество $B_1$~--- борелевское, так как функция $g$ борелевская. Но тогда $g^{-1}(B) = \xi^{-1}(B_1)$. Это множество по свойству измеримости случайной величины $\xi$ принадлежит $\mathcal{F}$, т.е. является событием. 
\end{proof}
...
\begin{defn}
    ...
    \begin{enumerate}
        \item ${\mathbb{E}\xi^k}$~--- {\it момент порядка $k$ или $k$-м момент} случайной величины $\xi$;
        \item ${\mathbb{E}|\xi|^k}$~--- {\it абсолютный $k$-м момент};
        \item ${\mathbb{E}(\xi - \mathbb{E}\xi)^k}$~--- {\it центральныq $k$-м момент};
        \item ${\mathbb{E}|\xi - \mathbb{E}\xi|^k}$~--- {\it абсолютный центральный $k$-м момент} случайной величины $\xi$
    \end{enumerate}
\end{defn}
