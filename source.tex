...
\begin{defn}
    {\it Борелевская функция}~--- функция $f: \mathbb{R} \mapsto \mathbb{R}$:
    \begin{equation*}
...
\begin{exmp}
    Функция Дирихле $D: \mathbb{R} \mapsto \{0,1\}$
    \begin{equation*}
...
\begin{defn}
    Функция $\xi$: $\Omega \mapsto \mathbb{R}^{(n)}$ называется {\it измеримой}, если полный прообраз борелевского множества $B$ лежит в $\sigma$-алгебре, т.е. 
    \begin{equation*}
...
\begin{enumerate}
    ...
    \begin{proof}
        $\mathbb{D}\xi = 0 \Rightarrow (\xi - a)^2 = 0, \xi = a = const.$ И наоборот: если $\xi = c$, то $\mathbb{D} \xi=\mathbb{E}(c-\mathbb{E} c)^{2}=\mathbb{E} 0=0.$
    \end{proof} 
...
\begin{defn}
    ...
    \begin{equation*}
        \mathbb{E}\left(\frac{\xi - \mathbb{E}\xi}{\sqrt{\mathbb{D}\xi}}\right)^4
    \end{equation*}
...
\begin{equation*}
    \sup\limits_{x}\left|P\left(\frac{p}{a} S_{N}<x\right)-G(x)\right| \underset{p \rightarrow 0}{\longrightarrow} 0,
\end{equation*}
...
\begin{equation*}
    \sup\limits_{x}\left|P\left(\frac{p}{a} S_{N}<x\right)-G(x)\right| \leqslant \frac{p b^{2}}{(1-p) a^{2}}
\end{equation*}
\end{namedthm}
...
\begin{equation*}
    ...
    = \int_{-\infty}^{x_{1}}\left(\int_{-\infty}^{\infty} f(x, y) d y\right) d x
...
\begin{proof}\par
    ...
    $$1 \geqslant \mathrm{P}(B) \geqslant \mathrm{P}(A)=\mathrm{P}(N(\omega, \varepsilon)<n)=F_{N(\varepsilon, \omega)}(n) \xrightarrow[n \rightarrow +\infty]{} 1.$$ по свойству функции распределения. Таким образом, было получено, что $\mathbb{P}(B) \rightarrow 1$, т.е. $\xi_n \xrightarrow[]{\text{p}} \xi.$
    
    \framebox[1.1\width]{$\text{п.н.} \nLeftarrow \text{p}$}
    
    Положим $\xi_{2^{k}}=I\left(\left[0, \frac{1}{2^{k}}\right]\right), \xi_{2^{k}+p}=I\left(\left[\frac{p}{2^{k}}, \frac{p+1}{2^{k}}\right]\right), 1 \leqslant p<2^{k}.$ Тогда $\xi_n \xrightarrow[]{\text{p}} 0$, т.к. $\mathbb{P}(\xi_n > 0) \leqslant$ длина отрезка в индикаторе $\leqslant \frac{2}{n} \rightarrow 0$, но $\xi_n \overset{\text{п.н.}}{\rightarrow} 0$, т.к. $\forall \omega \exists$ бесконечно много $n$, таких что $\xi_n(\omega) = 1.$
    ...
    $\Omega = [0,1], \mathcal{F} = \mathcal{B}([0,1])$, $\mathbb{P}$ - равномерное распределение. Определим для $k \geqslant 1 \quad \xi_k = 2^{k-1} I\left(\left[0, \frac{1}{2^{k-1}}\right]\right).$ Тогда $\forall k \quad \mathbb{E}\xi_k = 1$, но $\xi = I(\omega = 0)$.
    ...
    $$\left|\mathrm{E} f\left(\xi_{n}\right)-\mathrm{E} f(\xi)\right|=\left|\mathrm{E}\left(f\left(\xi_{n}\right)-f(\xi)|\leqslant \mathrm{E}| f\left(\xi_{n}\right)-f(\xi) |=\right.\right.$$
    $$=\mathrm{E}\left|f\left(\xi_{n}\right)-f(\xi)\right|\left(I_{A_{1}}+I_{A_{2}}+I_{A_{3}}\right) \leq$$
    $$\leqslant \frac{\varepsilon}{3} \mathrm{P}\left(A_{1}\right)+2 C\left(\mathrm{P}\left(A_{2}\right)+\mathrm{P}\left(A_{3}\right)\right) \leqslant \frac{\varepsilon}{3}+2 C\left(\frac{\varepsilon}{6 C}+\frac{\varepsilon}{6 C}\right)=\varepsilon,$$
...
\begin{enumerate}
    ...
    \item $\mathrm{E} \chi^{2}=k, \quad \mathrm{D} \chi^{2}=2 k$
    \item Пусть дана последовательность случайных величин $\chi_{n}^{2}$. Тогда при $n \to \infty$:
    \begin{equation*}
        \frac{\chi_{n}^{2}}{n} \stackrel{\mathrm{p}}{\longrightarrow} 1, \quad \frac{\chi_{n}^{2}-n}{\sqrt{2 n}} \Rightarrow \mathrm{N}_{0,1}
    \end{equation*}
...
\begin{enumerate}
    \item $\sqrt{n} \frac{\bar{X}-a}{\sigma} \in \mathrm{N}_{0,1}$
    \item $\frac{(n-1) S_{0}^{2}}{\sigma^{2}}=\sum_{i=1}^{n} \frac{\left(X_{i}-\bar{X}\right)^{2}}{\sigma^{2}} \in \mathrm{H}_{n-1}$
    \item Случайные величины $\bar{X}$ и $S_{0}^{2}$ независимы.
\end{enumerate}
\end{namedthm}
\begin{crlr}
    Пусть случайные величины $\xi_1, \ldots, \xi_n$ независимы и $\xi_i \sim \mathbf{N}(a,\sigma^{2})$. Тогда:
    \begin{enumerate}
        \item $\sqrt{n} \frac{\bar{X}-a}{\sigma} \in \mathrm{N}_{0,1}$ (ля $a$ при известном $\sigma^{2}$)
        ...
        \item $\sqrt{n} \frac{\bar{X}-a}{S_{0}} \in \mathrm{T}_{n-1}$ (для $a$ при неизвестном $\sigma^{2}$)
    \end{enumerate}
\end{crlr}
...
\begin{enumerate}
    ...
    \begin{equation*}
        \mathrm{P}\left(\overline{X}-\frac{\tau \sigma}{\sqrt{n}}<a<\overline{X}+\frac{\tau \sigma}{\sqrt{n}}\right)=\alpha, \quad \text { где } \quad \varphi_{0,1}(\tau)=\frac{1 + \alpha}{2}
    \end{equation*}
    ...
    \begin{equation*}
        \frac{(n-1) S_{0}^{2}}{\sigma^{2}} \in \mathrm{H}_{n-1}, \quad \text { где } \quad S_{0}^{2}=\frac{1}{n-1} \sum_{i=1}^{n}\left(X_{i}-\bar{X}\right)^{2}
    \end{equation*}
    ...
    \begin{equation*}
        \alpha=\mathrm{P}\left(g_{1}<\frac{(n-1) S_{0}^{2}}{\sigma^{2}}<g_{2}\right)=\mathrm{P}\left(\frac{(n-1) S_{0}^{2}}{g_{2}}<\sigma^{2}<\frac{(n-1) S_{0}^{2}}{g_{1}}\right)
    \end{equation*}
    \item ДИ для $a$ при неизвестном $\sigma^{2}$:
    \begin{equation*}
        \sqrt{n} \frac{\bar{X}-a}{S_{0}} \in \mathrm{T}_{n-1}
    \end{equation*}
    Пусть $c$~--- квантиль распределения $\mathbf(T)(n-1)$ уровня $\frac{1-\alpha}{2}$. Распределение Стьюдента симметрично, поэтому
    \begin{equation*}
        \alpha=\mathrm{P}\left(-c<\sqrt{n} \frac{\bar{X}-a}{S_{0}}<c\right)=\mathrm{P}\left(\bar{X}-\frac{c S_{0}}{\sqrt{n}}<a<\bar{X}+\frac{c S_{0}}{\sqrt{n}}\right)
    \end{equation*}
\end{enumerate}
