...
\begin{proof}
    $[\Leftarrow]$ Пусть $\mathcal{D} = \{D \,|\, D \in \mathfrak{B}, \xi^{-1}(D) \in \mathcal{A} \}$. Тогда $\mathcal{E} \subseteq \mathcal{D}$. Далее, в силу свойств прообразов и случайной величины $\xi$:
    \begin{gather*}
...
\begin{enumerate}
    \item Математическое ожидание:
    \begin{multline*}
        \mathrm{E} \xi=\sum_{k=1}^{\infty} k p q^{k-1}=p \sum_{k=1}^{\infty} k q^{k-1}=p \sum_{k=1}^{\infty} \frac{d q^{k}}{d q} = \\
        = p \frac{d}{d q}\left(\sum_{k=1}^{\infty} q^{k}\right)=p \frac{d}{d q}\left(\frac{q}{1-q}\right)=p \frac{1}{(1-q)^{2}}=\frac{1}{p}
    \end{multline*}
    \item Дисперсия:
    \begin{multline*}
        \mathrm{E} \xi(\xi-1)=\sum_{k=1}^{\infty} k(k-1) p q^{k-1}=p q \sum_{k=0}^{\infty} \frac{d^{2} q^{k}}{d q^{2}} =p q \frac{d^{2}}{d q^{2}}\left(\sum_{k=0}^{\infty} q^{k}\right) = \\
        =p q \frac{d^{2}}{d q^{2}}\left(\frac{1}{1-q}\right)=p q \frac{2}{(1-q)^{3}}=\frac{2 q}{p^{2}} \\
        \mathrm{D} \xi=\mathrm{E} \xi(\xi-1)+\mathrm{E} \xi-(\mathrm{E} \xi)^{2}=\frac{2 q}{p^{2}}+\frac{1}{p}-\frac{1}{p^{2}}=\frac{2 q-1+p}{p^{2}}=\frac{q}{p^{2}}
    \end{multline*}
\end{enumerate}
...
\begin{equation*}
    \mathrm{E} \xi^{k}=\int_{-\infty}^{\infty} x^{k} f_{\xi}(x) d x=\int_{0}^{\infty} x^{k} \lambda e^{-\lambda x} d x=\frac{1}{\lambda^{k}} \int_{0}^{\infty}(\lambda x)^{k} e^{-\lambda x} d(\lambda x)=\frac{k !}{\lambda^{k}}
\end{equation*}
...
\begin{enumerate}
    ...
    \item Дисперсия:
    \begin{equation*}
        \mathrm{E} \xi^{2}=\frac{2}{\lambda^{2}}, \quad \mathrm{D} \xi=\mathrm{E} \xi^{2}-(\mathrm{E} \xi)^{2}=\frac{1}{\lambda^{2}}
    \end{equation*}
\end{enumerate}
...
$$\mathrm{E} \xi=\int_{-\infty}^{\infty} x f_{\xi}(x) d x=\frac{1}{\sqrt{2 \pi}} \int_{-\infty}^{\infty} x e^{-x^{2} / 2} d x=0,$$
так как под интегралом стоит нечётная функция. Далее,

$$\mathrm{E} \xi^{2}=\frac{1}{\sqrt{2 \pi}} \int_{-\infty}^{\infty} x^{2} e^{-x^{2} / 2} d x=\frac{2}{\sqrt{2 \pi}} \int_{0}^{\infty} x^{2} e^{-x^{2} / 2} d x=-\frac{2}{\sqrt{2 \pi}} \int_{0}^{\infty} x d e^{-x^{2} / 2}=$$ $$=-\left.\frac{2 x}{\sqrt{2 \pi}} e^{-x^{2} / 2}\right|_{0} ^{\infty}+2 \int_{0}^{\infty} \frac{1}{\sqrt{2 \pi}} e^{-x^{2} / 2} d x=0+\int_{-\infty}^{\infty} \frac{1}{\sqrt{2 \pi}} e^{-x^{2} / 2} d x=1.$$
...
\begin{equation*}
    \overline{g(\mathbf{X})}=\frac{1}{n} \sum_{i=1}^{n} g\left(X_{i}\right) \xrightarrow[]{\mathbb{P}} \mathbb{E} g\left(X_{1}\right)=h(\theta)
\end{equation*}
