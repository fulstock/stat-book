...
\begin{titlepage}
    ...
    \vskip0.5cm
    {\upshape\large
%        Васильев Р., Рожков И., Рыгин А., Селезнёв М.
    }    
    \vfill
...
\begin{enumerate}
    ...
    $ \Rightarrow \myprob{\bigcup\limits_{i=1}^\infty A_i} = \sum\limits_{i=1}^\infty \myprob{A_i}$ ({\it счётная аддитивность})
    
    {\it (т.е. вероятность счётного объединения несовместных событий 
...
\begin{rmrk}
    Интуитивно понятно, что из счётной аддитивности следует и конечная аддитивность. Покажем это строго. Для любых несовместных $A_1, A_2, ..., A_n$ мы можем рассмотреть бесконечную последовательность $A_1, A_2, ..., A_n, \varnothing, \varnothing, ...$ . $\forall \; i \ne j \; A_i A_j = \varnothing$, так как наши $n$ событий несовместны, а пересечение любого множества с пустым множеством тоже является пустым множеством. Тогда \begin{equation*}
        \myprob{\bigcup\limits_{i=1}^\infty A_i} = \sum\limits_{i=1}^\infty \myprob{A_i} = \{\myprob{\varnothing} = 0\}
...
\begin{enumerate}
    \item $\forall A \in \mathcal{F}~ \myprob{A|B} \geqslant 0, \text{т.к.}~ \myprob{AB} \geqslant 0,~ \myprob{B} > 0$
    ...
    \begin{multline*}
        \mathbb{P}\left(\left(\bigcup\limits_{i=1}^\infty A_i\right)|B\right)  = \cfrac{\mathbb{P}\left(\left(\bigcup\limits_{i=1}^\infty A_i\right) \cap B\right)}{\myprob{B}} = \cfrac{\mathbb{P}\left(\bigcup\limits_{i=1}^\infty\left(A_i \cap B\right)\right)}{\myprob{B}} = \cfrac{\sum\limits_{i=1}^\infty \myprob{A_i \cap B}}{\myprob{B}} = \\
...
Т.е. события независимы в совокупности, если вероятность одновременного наступления любого набора из этих событий равна произведению вероятностей событий, входящих в этот набор. 

В частности, при $n = 2$: события $A$ и $B$ независимы, если $\myprob{A \cap B} = \myprob{A}\myprob{B}$.
\end{defn}
...
\begin{proof}
\begin{enumerate} 
    \item Действительно, $AB = \varnothing \Rightarrow \myprob{AB} = 0.$ Но $ \myprob{A}\myprob{B} = 0 * \myprob{B} = 0 \Rightarrow \myprob{AB} = \myprob{A} \myprob{B}.$
    
    Аналогично для $\myprob{A} = 0$.
    ...
    \item Предположим, что события независимы. Тогда $\myprob{AB} = \myprob{A}\myprob{B},$ но в силу вложенности $A$ в $B$ $\myprob{AB} = \myprob{A}$, следовательно, $\myprob{B} = 1$, что противоречит условию.
    \item Действительно, 
    \begin{equation*}
...
\begin{namedthm}[Формулы Байеса]
    Пусть дано событие $A$ и множества $H_1, \ldots, H_n, \ldots$; $\myprob{H_i} > 0$, причём $H_i \cap H_j = \varnothing ~(i \neq j)$ и $\bigcup\limits_{i=1}^\infty H_i \supset A$ (например, $\bigcup\limits_{i=1}^{\infty}H_i = \Omega$). Пусть также $\myprob{A} > 0$. Тогда справедливы {\it формулы Байеса} для $i = 1, 2, \ldots$:
    \begin{equation*}
        \mathbb{P}\left(H_{i} | A\right)= \frac{\mathbb{P}\left(H_{i}\right) \cdot \mathbb{P}\left(A | H_{i}\right)}{\sum\limits_{j=1}^{\infty} \mathbb{P}\left(H_{j}\right) \cdot \mathbb{P}\left(A | H_{j}\right)}
    \end{equation*}
\end{namedthm}
...
\begin{defn}
    ...
    Т.е. борелевская функция - это функция, для которой прообраз (множество $f^{-1}(B) = \{x : f(x) \in B\}$ ) любого борелевского множества - тоже борелевское множество.
\end{defn}
...
В самом деле, прообразом любого борелевского множества ${A: 1 \in A, 0 \notin A}$ является множество рациональных чисел; прообразом борелевского множества ${B: 0 \in B, 1 \notin B}$ является множество иррациональных чисел; прооборазом борелевского множества ${C: 0 \in C, 1 \in C}$ является вся вещественная прямая, а прообразом борелевского множества ${E: 0 \notin E, 1 \notin E}$ является пустое множество. Но $\mathbb{R}, \> \mathbb{Q}, \> \mathbb{I}, \> \varnothing $~--- борелевские множества, а значит, выполняется определение борелевской функции.
\end{exmp}
...
\begin{defn}
    Пусть даны $(\Omega, \mathcal{F})$~--- измеримое пространство и $(\mathbb{R}, \mathfrak{B})$, где $\mathfrak{B}$~--- борелевская $\sigma$-алгебра множеств на числовой прямой $\mathbb{R}$. Тогда измеримая относительно $\mathcal{F}$ функция $\xi: \Omega \to \mathbb{R}$ называется {\it случайной величиной}.
\end{defn}
...
\end{array}\right.
\end{equation*}
$\Omega = [0; 1], \mathcal{F} = \{\varnothing, \Omega\}$~--- минимальная $\sigma$-алгебра.  

Чтобы показать неизмеримость, достаточно найти такое борелевское множество, прообраз которого не будет принадлежать $\sigma$-алгебре. В данном случае $\mathcal{F}$ состоит всего лишь из двух множеств - $\{[0; 1], \varnothing\}$.

Как и в примере с функцией Дирихле, попробуем перебрать борелевские множества, содержащие значения $\xi(\omega)$. Тогда мы увидим, что для любого борелевского множества $A: 0 \in A, 1 \notin A$ ~--- например, множества ${A_1 = (-\infty, \frac{1}{3})}$ ~--- прообразом является множество $(\frac{1}{2}, 1]$. Но это множество не входит в $\mathcal{F}$, а значит, $\xi(\omega)$ неизмерима.

Отсюда можно сделать несколько выводов. Во-первых, измеримость функции зависит от выбора $\sigma$-алгебры. Например, если мы рассмотрим ту же функцию $\xi(\omega)$ на том же $\Omega = [0; 1]$, но с другой $\sigma$-алгеброй ${\widehat{\mathcal{F}} = \{[0; 1], [0; \frac{1}{2}], (\frac{1}{2}, 1], \varnothing\}}$, то наша функция будет измеримой, а следовательно ~--- случайной величиной.
...
\begin{proof}
    $\boxed{\Leftarrow}$ Пусть $\mathcal{D} = \{D : D \in \mathfrak{B}, \xi^{-1}(D) \in \mathcal{F} \}$. Тогда $\mathcal{E} \subseteq \mathcal{D}$. Далее, в силу свойств прообразов и случайной величины $\xi$:
    \begin{equation*}
...
\begin{crlr}
    $\xi$~--- случайная величина $\Leftrightarrow$ $\forall x \in \mathbb{R}:~ \{\omega : \xi(\omega) < x \} \in \mathcal{F}$. Причем вместо знака $<$ может стоять любой другой знак неравенства, как строгого, так и нестрогого.
\end{crlr}
...
\begin{defn}
    ...
    \begin{equation*}
        \mathbb{P}_\xi(B) = \myprob{\xi^{-1}(B)} = \myprob{\xi \in B}
    \end{equation*}
\end{defn}

\begin{rmrk}
    Можно рассматривать распределение как композицию отображений. Если $\xi: \Omega \mapsto \mathbb{R}$, то полный прообраз - это отображение $\xi^{-1}: \mathfrak{B} \mapsto \mathcal{F}$. В свою очередь, $\mathbb{P}: \mathcal{F} \mapsto \mathcal{R}$. Тогда $$\mathbb{P}_{\xi} = \mathbb{P} \circ \xi^{-1}, \quad \mathbb{P}_{\xi}: \mathfrak{B} \mapsto \mathbb{R}.$$
\end{rmrk}
...
\begin{enumerate}
    ...
    \item $F_\xi(x)$ монотонно неубывает (т.е. для $x_1 < x_2 \Rightarrow F_\xi(x_1) \leqslant F_\xi(x_2))$;
...
\begin{enumerate}
    ...
    \item 
        ...
        Теперь докажем, что $F_\xi(n) \xrightarrow[n \rightarrow +\infty]{} 1 \Leftrightarrow 1 - F_\xi(n) = \myprob{\xi \geqslant n} \xrightarrow[n \rightarrow +\infty]{} 0$.
    
        Аналогично, пусть $B_n = \{\xi \geqslant n\}; B_{n+1} = \{ \xi \geqslant (n+1) \} \subseteq B_n, ...$
        \begin{equation*}
...
{\bf Задача.}
 Пусть есть не более чем счётное множество элементарных исходов $\Omega$. Рассмотрим функции ${f(\omega) : \Omega \mapsto \mathbb{R}}$.
 
 Какая функция всегда измерима (т.е. измерима относительно любой ${\sigma \text{-алгебры}}$)? 
 
 Относительно какой $\sigma$-алгебры измерима любая функция ${f(\omega) : \Omega \mapsto \mathbb{R}}$?
...
\begin{enumerate}
    ...
    (кроме, может быть, множества меры $0$ ~--- например, функция равномерного распределения недифференцируема в точках $0$ и $1$)
...
Пример сингулярной функции распределения - Канторова лестница.

\begin{namedthm}[Теорема Лебега о разложении функции распределения]
    Пусть $\xi$~--- случайная величина с функцией распределения $F_\xi(x).$ Тогда существуют и единственны три функции $F_{ac}(x), F_s(x), F_d(x)$, соответственно абсолютно непрерывная, сингулярная и дискретная функции распределения, три числа $p_1, p_2, p_3 \geqslant 0, p_1 + p_2 + p_3 = 1$ такие, что 
    \begin{equation*}
...
\begin{enumerate}
    ...
    \begin{proof} 
        Неотрицательность $\xi$ означает, что $a_i \geqslant 0$ при всех $i$ в случае дискретного распределения, либо $f_\xi(x) = 0$ при $x < 0$ - для абсолютно непрерывного распределения. И в том, и в другом случае имеем:
    ...
    \begin{proof}
        В равенстве (1) заменим сложение умножением и плотность совместного распределениея произведением плотностей (это возможно в силу независимости случайных величин):
        $$\begin{aligned}
...
\begin{defn}
    Число $\sigma = \sqrt{\mathbb{D}\xi}$ называют {\it среднеквадратичным отклонением} случайной величины $\xi$.
\end{defn} 
...
\begin{defn}
    ...
    \end{equation*}
    Характеризует <<скошенность>> графика: \medskip\hfill\break
    \begin{center}
    \begin{tikzpicture}
...
\begin{defn}
    ...
    \end{equation*}
    Характеризует <<островершинность>> графика:
    \medskip\hfill\break
    \begin{center}
    \begin{tikzpicture}
    \begin{axis}[skewkurt]
...
\begin{rmrk}
    Слагаемое $-3$ добавлено, чтобы коэффициент эксцесса стандартного нормального распределения был равен нулю. Иногда его не учитывают и считают, что коэффициент эксцесса стандартного нормального распределения равен 3.
\end{rmrk}
...
\begin{defn}
    Пусть, для простоты, функция распределения $F$ непрерывна и строго монотонна. Тогда {\it квантилью} уровня (порядка) $\delta$, где $\delta \in (0; 1), $ называется решение $x_\delta$ уравнения $F(x_\delta) = \delta.$ 
\end{defn} 
...
\begin{rmrk}
    Медиана является квантилью уровня $\frac{1}{2}.$
\end{rmrk} 
...
\begin{enumerate}
    ...
    \begin{proof}
        Рассмотрим $\eta = \xi - \mathrm{Med}\xi$. Очевидно, что $\mathrm{Med}\eta = 0$. Тогда нам надо показать, что $\forall~c \in \mathbb{R} $ справедливо 
        ...
        Так как $\mathrm{Med}\eta = 0, \quad$ $\mathbb{P}(\eta \leqslant 0) = \mathbb{P}(\eta \geqslant 0) = \frac{1}{2}$.
        
        Отсюда вытекает, что $$ \mathbb{E}\left[~|\eta - c| - |\eta|~\right] \geqslant 0.$$
...
\begin{proof}
    ...
    Далее, т.к. $\left(1-p_{n}\right)^{n}=\left(1-\frac{n p_{n}}{n}\right)^{n} \xrightarrow[n \rightarrow +\infty]{} e^{-\lambda}$, то $(1 - p_n)^{nx} \xrightarrow[n \rightarrow +\infty]{} e^{-\lambda x}.$ По определению, $\lfloor n x\rfloor \leqslant n x<\lfloor n x\rfloor+1$ или, что эквивалентно, $n x-1<\lfloor n x\rfloor \leqslant n x.$ и таким образом верно $(1 - p_n)^{\lfloor nx \rfloor} \xrightarrow[n \rightarrow +\infty]{} e^{-\lambda x}$. Следовательно, $F_n(x) \xrightarrow[n \rightarrow +\infty]{} 1 - e^{-\lambda x}$, что есть функция показательного распределения. 
\end{proof}
...
\begin{proof}\par
    ...
    \framebox[1.1\width]{$\text{п.н.} \Rightarrow \text{p}$}
    Ограничимся для простоты случаем, когда $\xi_n(\omega) \rightarrow \xi(\omega)$ для любого $\omega$. Зафикисируем $\omega \in \Omega.$ По определению предела, $\xi_n(\omega) \xrightarrow[n \rightarrow +\infty]{} \xi(\omega)$, если для всякого $\varepsilon > 0$ найдётся $N = N(\omega, \varepsilon) \geqslant 0$ такое, что для всех $n > N$ выполняется неравенство $|\xi_n(\omega) - \xi(\omega)| < \varepsilon$.
    
    Событие $A = \{ n > N(\omega,\varepsilon) \}$ влечёт событие $B = \{ |\xi_n(\omega) - \xi(\omega)| < \varepsilon \}$. Тогда 
    $$1 \geqslant \mathbb{P}(B) \geqslant \mathbb{P}(A)=\mathbb{P}(N(\omega, \varepsilon)<n)=F_{N(\varepsilon, \omega)}(n) \xrightarrow[n \rightarrow +\infty]{} 1.$$ по свойству функции распределения. Таким образом, было получено, что $\mathbb{P}(B) \rightarrow 1$, т.е. $\xi_n \xrightarrow[]{\text{p}} \xi.$
    
    \framebox[1.1\width]{$\text{п.н.} \nLeftarrow \text{p}$}
    ...
    \framebox[1.1\width]{$\text{p} \Rightarrow \text{d}$}
...
Прежде чем вычислять статистику $\overline{\chi}^{2}$, мы объединили 4 последних события с низкими частотами в одно и пересчиталм новые теоретические вероятности $\tilde{p}_i^{(0)}$ и, соответственно, новые ожидаемые значения. В этом случае $\overline{\chi}^{2} \approx 1,05$. Т.к. $k=5$, то по таблице распределения $\chi^{2}$ находим соответствующий уровень значимости $\alpha = 0,79$. Гипотеза о низкой точности стрельбы принимается.
...
\begin{crlr}
    Пусть случайные величины $\xi_1, \ldots, \xi_n$ независимы и $\xi_i \sim \mathbf{N}(a,\sigma^{2})$. Тогда:
    \begin{enumerate}
        \item $\sqrt{n} \frac{\overline{X}-a}{\sigma} \in \mathrm{N}_{0,1}$ (ля $a$ при известном $\sigma^{2}$)
...
\begin{thebibliography}{1}
	...
	\bibitem{ushakov_tvims}
		В.Г. Ушаков.
		{\it Теория вероятностей и математическая статистика},
		2014
